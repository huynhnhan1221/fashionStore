			...
			<div class="admin-left-content-box">
              ...
              <form action="UpdateProductServlet" name="addproduct" method="post">
                ...
                %>
                        ...
                        <tr>
                            <td>Tên: &nbsp;</td>
                            <td><input required="true" class="textfield" type="text" name="productname" value="<% if(productID!=null) out.print(editproduct.getName()); %>"/></td>
                        </tr>
                        <tr>
                            <td>Mô tả: &nbsp;</td>
                            <td><input class="textfield" type="textarea" name="productdes" rows="3" cols="40" value="<% if(productID!=null) out.print(editproduct.getDescribes()); %>"/></td>
                        </tr>
                        <tr>
                            <td>Đơn giá: &nbsp;</td>
                            <td><input required="true" class="textfield" type="number" name="productprice" value="<% if(productID!=null) out.print(editproduct.getPrice()); %>"/>,000 VNĐ</td>
                        </tr>
                        <tr>
                            <td>Số lượng: &nbsp;</td>
                            <td><input required="true" class="textfield" type="number" name="prodQuantity" value="<% if(productID!=null) out.print(editproduct.getQuantity()); %>"/> cái</td>
                        </tr>
                        <tr>
                            <td>Hình ảnh: &nbsp;</td>
                            <td>
                                <input class="textfield" type="text" name="productimage" id="filename" value="<% if(productID!=null) out.print(editproduct.getImage()); %>"/>
...
%>
                <% if(products.size()>5){  %>
                <p>
                    <a style="border-radius: 2px;text-decoration: none;background-color: #dccfc6" href="<% if(pagenumb>1) out.print("product.jsp?page=" + (pagenumb-1));%>">Trang trước</a>&nbsp;
                    <% for(int i=1;i<=products.size()/6+1;i++) { %>
                            <a style="border-radius: 2px;text-decoration: none;background-color: #dccfc6;" href="<% if (pagenumb!=i) out.print("product.jsp?page=" + i);%>">
                                <% if(pagenumb==i) out.print("<strong>"+i+"</strong>");
                                       else out.print(i);
                                %>
                            </a>&nbsp;
                        <% } %>
                    <a style="border-radius: 2px;text-decoration: none;background-color: #dccfc6" href="<% if(pagenumb<products.size()/10+1) out.print("product.jsp?page=" + (pagenumb+1));%>">Trang sau</a>&nbsp;
                </p>
                <% } %>
                </table>
				</div>
